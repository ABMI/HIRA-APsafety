...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (372892,373764,4047745,373995)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (372892,373764,4047745,373995)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (432586,40480615)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (432586,40480615)
  and c.invalid_reason is null
...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4174669,4145786,4082850,4118897,4082996,4086417)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (4174669,4145786,4082850,4118897,4082996,4086417)
  and c.invalid_reason is null
...
WHERE DATEDIFF(d,C.visit_start_date, C.visit_end_date) <= 30
AND YEAR(C.visit_start_date) - P.year_of_birth >= 60
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= P.OP_END_DATE AND A.visit_occurrence_id = P.visit_occurrence_id ) cc 
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,0,P.START_DATE) AND A.START_DATE <= P.OP_END_DATE AND A.visit_occurrence_id = P.visit_occurrence_id ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
GROUP BY p.person_id, p.event_id
HAVING COUNT(cc.event_id) = 0
-- End Correlated Criteria
